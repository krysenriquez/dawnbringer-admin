...
} from '../../stores/ProductVariantInfoQueryProvider'
...
const ProductVariantEditForm = () => {
  ...
  const {refetch} = useProductVariantInfoQueryContext()
  ...
  const cancel = () => {
    navigate(`/product-variants/${productVariantInfo.sku}`, {
      state: {sku: productVariantInfo.sku},
    })
  }
    ...
    swal
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          try {
            const {data: response} = await updateProductVariant(formData)
            swal.fire('Product Variant Updated', response.detail, 'success')
          } catch (ex) {
                                      ...
                                      <div className='row mb-5' key={level.membershipLevelLabel}>
                                        ...
                                        <div className='col-8'>
                                          <InputField
                                            ...
                                            placeholder={
                                              level.membershipLevelLabel + ' Point Value'
                                            }
                                            required
                                          />
                                        </div>
